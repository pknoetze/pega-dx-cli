...
import { createPegaApiClient, type PegaApiClient, type RequestOpts } from './lib/api-client.js';
...
export abstract class BaseCommand extends Command {
  ...
  protected async runPatch(flags: BaseFlags, path: string, body: unknown, opts?: RequestOpts): Promise<void> {
    const cfg = getConfig(flags.profile);
    const url = `${cfg.baseUrl}/prweb/api/application/v2${path}`;
    if (flags['dry-run']) {
    ...
    try {
      const client = await this.getClient(flags);
      const result = await client.patch(path, body, opts);
      this.emit(result, flags);
    } catch (err) {
      this.fail(err);
    }
