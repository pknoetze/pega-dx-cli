...
export default class UiListsCreatePersonalization extends BaseCommand {
  static override description = 'Create a personalization on a UI list';
  static override examples = ['<%= config.bin %> ui-lists create-personalization LIST-1 --name "My View"'];
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(UiListsCreatePersonalization);
