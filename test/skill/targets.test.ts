...
import { resolveTargetDest, listTargets, SKILL_TARGETS, type SkillTarget } from '../../src/skill/targets.js';
...
describe('resolveTargetDest', () => {
  ...
  it.each([
    ...
    const res = resolveTargetDest(target as SkillTarget, opts);
  ...
  it('throws INVALID_ARGS on unknown target', () => {
    expect(() => resolveTargetDest('bogus' as unknown as SkillTarget, opts)).toThrow(/INVALID_ARGS/);
  });
});
