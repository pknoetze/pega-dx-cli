...
const { default: DataGet } = await import('../../../src/commands/data/get.js');
...
describe('data get', () => {
  test('GETs /data_views/{dataViewId}', async () => {
    mockOAuthSuccess('https://pega.example.com');
    const scope = nock('https://pega.example.com')
      .get('/prweb/api/application/v2/data_views/D_MyDataView')
      .reply(200, { pxResults: [] });

    captured = captureOutput();
    await DataGet.run(['D_MyDataView']);
  ...
  test('URL-encodes the dataViewId', async () => {
    ...
    const scope = nock('https://pega.example.com')
      .get(`/prweb/api/application/v2/data_views/${encodedId}`)
      .reply(200, { pxResults: [] });
    captured = captureOutput();
    await DataGet.run([idWithSpace]);
    expect(scope.isDone()).toBe(true);
  });

  test('--dry-run emits GET method with correct URL and no network call', async () => {
    captured = captureOutput();
    await DataGet.run(['D_MyDataView', '--dry-run']);
    const out = JSON.parse(captured.stdout.join(''));
    expect(out.method).toBe('GET');
    expect(out.url).toBe(
      'https://pega.example.com/prweb/api/application/v2/data_views/D_MyDataView',
    );
  });
});
