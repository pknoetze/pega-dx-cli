...
import SocialSearchTags from '../../../src/commands/social/search-tags.js';
...
describe('social search-tags', () => {
  test('happy path with no flags returns tags (no query string)', async () => {
    mockOAuthSuccess('https://pega.example.com');
    const scope = nock('https://pega.example.com')
      .get('/prweb/api/application/v2/tags')
      .reply(200, { tags: [] });

    captured = captureOutput();
    await SocialSearchTags.run([]);
    expect(scope.isDone()).toBe(true);
    expect(JSON.parse(captured.stdout.join(''))).toEqual({ tags: [] });
  });

  test('both flags produce correct query string', async () => {
    mockOAuthSuccess('https://pega.example.com');
    const scope = nock('https://pega.example.com')
      .get('/prweb/api/application/v2/tags')
      .query({ searchFor: 'security', listSize: '20' })
      .reply(200, { tags: ['security'] });

    captured = captureOutput();
    await SocialSearchTags.run(['--search-for', 'security', '--list-size', '20']);
    expect(scope.isDone()).toBe(true);
  });

  test('--dry-run emits GET with correct URL', async () => {
    captured = captureOutput();
    await SocialSearchTags.run(['--search-for', 'security', '--dry-run']);
    const out = JSON.parse(captured.stdout.join(''));
    expect(out.method).toBe('GET');
    expect(out.url).toContain('/prweb/api/application/v2/tags');
    expect(out.url).toContain('searchFor=security');
  });
  ...
  test('404 emits structured error', async () => {
    ...
    captured = captureOutput();
    await expect(SocialSearchTags.run([])).rejects.toThrow();
